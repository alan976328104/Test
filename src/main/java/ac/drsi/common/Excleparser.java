...
public class Excleparser {
    private static final Logger logger = LoggerFactory.getLogger(Excleparser.class);
     ...
     * 读取数据
     */
    private List<OpenCSV> datas = new ArrayList<>();
     ...
     */
    public Excleparser parse(InputStream stream,Integer begin,Integer end)
     ...
     * @throws ParseException
     */
    public synchronized Excleparser parse(InputStream stream, int sheetId)
            throws InvalidFormatException, IOException, ParseException {
        // 每次转换前都清空数据
        datas.clear();
        // 打开表格文件输入流
     ...
     * @return 表格读取数据
     */
    public List<OpenCSV> getDatas(boolean dropFirstRow) {
    ...
    class DefaultSheetHandler implements ISheetContentHandler {
        ...
        @Override
        public void init(List<OpenCSV> datas) {
            this.datas = datas;
//          this.columsLength = columsLength;
        ...
        //将Excel第一行表头的列数当做数组的长度，要保证后续的行的列数不能超过这个长度，这是个约定。
            ...
            datas.add(new OpenCSV(readRow[0], readRow[1]));
            readRow = null;
            fristRow.clear();
        	}
        }
